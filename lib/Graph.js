...
Graph.prototype.removeNode = function(node) {
  ...
  /* 在删除关系的时候取一下开始的，开始的存在的话，处理一下其nodes
  //从各个存在node中删除对应的node
  for(let i=0;i<this.nodes.length;i++){
    let n = this.nodes[i]
    for(let j=0;j<n.nodesTo.length;j++){
      let onode = n.nodesTo[j];
      if(onode.id===node.id){
        n.nodesTo.splice(j,1);
        break;
      }
    }
  }
  ...
  for(let i=this.edges.length-1;i>-1;i--){
    ...
    if(edge.source.id===node.id || edge.target.id===node.id){
      ...
      if(edge.target.id===node.id){
        //需要从对应的node中删除nodesTo对应的node
        let nodeSource = this.getNode(edge.source.id);//取出source的node
        if(nodeSource){
          for(let j=0;j<nodeSource.nodesTo.length;j++){
            let onode = nodeSource.nodesTo[j];
            if(onode.id===node.id){
              nodeSource.nodesTo.splice(j,1);
              break;
            }
          }
        }
